...
def _external_evidence(retrieved_at: str) -> tuple[ResearchEvidenceItem, ...]:
    ...
    return (
        ...
        ResearchEvidenceItem("search_ai_monetization", "company_specific_research", "Search and AI monetization evidence", "Search growth reflected higher queries, advertiser spending, and ad-format/delivery improvements; AI Overviews and AI Mode expand product surfaces, but monetization parity is not assumed.", None, "2026", filing, "2026-07-23", retrieved_at),
...
def build_alphabet_research_profile(
    ...
    *,
    ...
    y1 = _assumption("year1_growth", 0.22, "A 22% DCF Year 1 growth rate translates the June-2026 TTM base toward the FY2026/FY2027 consensus levels while recognizing that the DCF year ends six months after FY2026. Search, Cloud and subscriptions support the run rate; one strong quarter is not annualized.", ("ttm_revenue", "fy1_consensus_revenue", "fy2_consensus_revenue", "q2_2026_revenue", "search_q2_growth", "cloud_q2_growth"))
    y2 = _assumption("year2_growth", 0.17, "Year 2 steps down as the Revenue base expands beyond the FY2027 consensus period. Cloud backlog and AI monetization support durability, while Search maturity, capacity delivery and comparison effects constrain extrapolation.", ("fy2_consensus_revenue", "cloud_backlog", "search_ai_monetization", "2026_capex_guidance"))
    y3 = _assumption("year3_growth", 0.13, "Year 3 begins normalization toward mature consolidated growth as Search scale, AI-interface competition, regulation and infrastructure intensity offset continuing Cloud and subscription mix benefits.", ("revenue_cagr_3y", "search_ai_disruption", "cloud_q2_growth", "technical_infrastructure_commitments"))
    ...
    mature_margin = _assumption("mature_operating_margin", 0.32, "A 32% mature margin stays near FY2025 economics but below the current TTM/Q2 level. Search operating leverage and Cloud mix offset, but do not erase, AI depreciation, infrastructure operating costs, Other Bets and shared AI R&D.", ("latest_annual_operating_margin", "ttm_operating_margin", "q2_2026_operating_margin", "services_q2_margin", "cloud_q2_margin", "h1_2026_depreciation", "other_bets_drag"))
    starting_stc = _assumption("starting_sales_to_capital", 0.45, "0.45x is anchored near the latest 0.44x accounting result and far below the former 0.8x provisional input, reflecting the current server/data-center build and assets not yet in service.", ("latest_sales_to_capital", "sales_to_capital_3y", "h1_2026_capex", "ttm_capex", "h1_2026_depreciation"))
    mature_stc = _assumption("mature_sales_to_capital", 0.60, "0.60x allows efficiency to recover as today's AI capacity is utilized, but remains below the normalized 3Y anchor and does not assume a return to historical asset-light economics.", ("sales_to_capital_3y", "h1_2026_capex", "2026_capex_guidance", "technical_infrastructure_commitments", "cloud_backlog"))
    ...
    terminal_roic = 0.32 * (1 - 0.17) * 0.60
    ...
    profile = CompanyResearchProfile(
        ...
        rationale="Unreviewed Alphabet issuer-level research candidate; valuation is output only and market price was not used.",
    ...
    ranges = (
        ResearchRange("year1_growth", 0.18, 0.22, 0.26, "TTM/fiscal-period mismatch and unusually strong Q2 growth define a broad, non-probabilistic evidence range.", y1.evidence_references),
        ResearchRange("year2_growth", 0.13, 0.17, 0.21, "Cloud backlog and AI monetization support durability while scale and delivery capacity constrain extrapolation.", y2.evidence_references),
        ResearchRange("year3_growth", 0.09, 0.13, 0.17, "No reliable third consensus year is used; Search maturity and AI competition dominate the range.", y3.evidence_references),
    )
    reconciliation = (
    ...
    return AlphabetResearchProfileResult(CompanyProfileLookupResult(profile, True, None), _revenue_rows(history, revenue_anchors, external, retrieved_at), _segment_rows(), ranges, current_assumptions, reconciliation, ("live_forward_consensus_is_supporting_evidence_not_an_assumption",))
