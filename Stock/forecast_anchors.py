...
@dataclass(frozen=True)
class ForwardEstimateEvidence:
    primary: ForwardRevenueEstimateSet | None
    references: tuple[ForwardRevenueEstimateSet, ...]
    warnings: tuple[str, ...] = ()


@dataclass(frozen=True)
class ForwardEstimateProviderResult:
    issuer_id: str
    source_ticker: str
    provider: str
    annual: ForwardRevenueEstimateSet | None
    quarterly: tuple[ForwardRevenueEstimate, ...]
...
@dataclass(frozen=True)
class ProviderRevenueComparison:
    ...
    primary_analyst_count: int | None
    reference_analyst_count: int | None
    primary_date_explicit: bool
    reference_date_explicit: bool
    warnings: tuple[str, ...] = ()


@dataclass(frozen=True)
class ForwardEstimateProviderAudit:
    ticker: str
    issuer_id: str
    provider: str
    annual_years_available: int
    quarterly_periods_available: int
    explicit_fiscal_dates: bool
    provider_as_of_available: bool
    analyst_counts_available: bool
    forward_ttm_feasibility: str
    warnings: tuple[str, ...]
    suitable_for_primary_use: bool
...
def assemble_forward_estimate_evidence(
    primary: ForwardRevenueEstimateSet | None,
    *references: ForwardRevenueEstimateSet,
) -> ForwardEstimateEvidence:
    """Keep conflicting providers separate; never average or overwrite them."""
    warnings = []
    if primary is None and references:
        warnings.append("primary_unavailable_using_reference_only")
    sources = ([primary] if primary is not None else []) + list(references)
    if len({item.source for item in sources}) > 1:
        warnings.append("multiple_sources_not_blended")
    return ForwardEstimateEvidence(primary, tuple(references), tuple(warnings))


def compare_forward_estimate_sources(
    primary: ForwardRevenueEstimateSet,
    reference: ForwardRevenueEstimateSet,
    *,
    material_difference_threshold: float = 0.05,
) -> tuple[ProviderRevenueComparison, ...]:
    """Compare matching fiscal endpoints without blending provider values."""
    if material_difference_threshold < 0:
        raise ValueError("material difference threshold must be non-negative")
    reference_by_period = {
        item.fiscal_period_end: item
        for item in reference.estimates
        if item.available and item.fiscal_period_end is not None
    }
    comparisons = []
    for item in primary.estimates:
        if not item.available or item.fiscal_period_end is None:
            continue
        other = reference_by_period.get(item.fiscal_period_end)
        if other is None:
            continue
        difference = item.revenue_estimate - other.revenue_estimate
        percentage = difference / other.revenue_estimate
        warnings = (
            ("forward_revenue_sources_materially_disagree",)
            if abs(percentage) > material_difference_threshold else ()
        )
        comparisons.append(ProviderRevenueComparison(
            item.fiscal_period_end, item.source, other.source,
            item.revenue_estimate, other.revenue_estimate, difference,
            percentage, item.analyst_count, other.analyst_count,
            item.fiscal_period_explicit, other.fiscal_period_explicit,
            warnings,
        ))
    return tuple(comparisons)


def assess_forward_ttm_feasibility(
    quarterly: tuple[ForwardRevenueEstimate, ...],
) -> tuple[Literal["feasible", "partially_feasible", "not_feasible"], str]:
    """Assess coverage only; this phase deliberately does not build forward TTM."""
    usable = sorted(
        item.fiscal_period_end for item in quarterly
        if item.available and item.fiscal_period_explicit
        and item.fiscal_period_end is not None
        and item.revenue_estimate is not None
    )
    if len(usable) < 4:
        status = "partially_feasible" if usable else "not_feasible"
        return status, "fewer_than_four_explicit_forward_quarters"
    latest_four = usable[:4]
    for left, right in zip(latest_four, latest_four[1:]):
        month_delta = (right.year - left.year) * 12 + right.month - left.month
        if month_delta not in {2, 3, 4}:
            return "partially_feasible", "quarterly_periods_not_consecutive"
    return "feasible", "four_consecutive_explicit_forward_quarters"


def audit_forward_estimate_provider(
    result: ForwardEstimateProviderResult,
) -> ForwardEstimateProviderAudit:
    annual = (
        tuple(item for item in result.annual.estimates if item.available)
        if result.annual is not None else ()
    )
    quarterly = tuple(item for item in result.quarterly if item.available)
    explicit = bool(annual) and all(item.fiscal_period_explicit for item in annual)
    counts = bool(annual) and all(item.analyst_count is not None for item in annual)
    calendar_mismatch = False
    if result.annual is not None:
        actual = result.annual.latest_actual_fiscal_period
        expected = actual
        for item in annual:
            expected = expected + pd.DateOffset(years=1)
            if (
                item.fiscal_period_end.month != expected.month
                or abs(item.fiscal_period_end.day - expected.day) > 7
            ):
                calendar_mismatch = True
                break
    feasibility, _ = assess_forward_ttm_feasibility(result.quarterly)
    warnings = list(result.warnings)
    if calendar_mismatch:
        warnings.append("provider_fiscal_calendar_mismatch")
    if not counts:
        warnings.append("analyst_count_coverage_incomplete")
    if not explicit:
        warnings.append("explicit_fiscal_date_coverage_incomplete")
    suitable = (
        result.available and len(annual) >= 3 and explicit and counts
        and not calendar_mismatch
    )
    return ForwardEstimateProviderAudit(
        result.source_ticker, result.issuer_id, result.provider,
        len(annual), len(quarterly), explicit,
        result.provider_as_of is not None, counts, feasibility,
        tuple(dict.fromkeys(warnings)), suitable,
    )
...
def alpha_vantage_revenue_estimates_to_set(
    *,
    ticker: str,
    payload: dict,
    latest_actual_fiscal_period: pd.Timestamp,
    ...
    """Normalize Alpha Vantage ``EARNINGS_ESTIMATES`` annual Revenue fields.

    This adapter is intentionally acquisition-free. A caller with an API key
    may pass the official JSON response. Annual records require explicit
    ``fiscalDateEnding``; analyst count and revision timestamps are retained
    when supplied. Provider error/information payloads produce no fabricated
    estimates.
    """
    raw_rows = _alpha_vantage_rows(payload, "annual")
    estimates = []
    for row in raw_rows:
        period = pd.to_datetime(
            row.get("date") or row.get("fiscalDateEnding")
            or row.get("fiscal_date_ending"),
            errors="coerce",
        )
        if pd.isna(period):
            period = None
        # Revision-history fields are relative estimate snapshots, not a
        # verified provider update timestamp. Keep provider_as_of unavailable.
        source_as_of = None
        estimates.append(ForwardRevenueEstimate(
            issuer_id=issuer_anchor_ticker(ticker),
            source_ticker=issuer_anchor_ticker(ticker),
            fiscal_period_end=period,
            fiscal_year_label=row.get("fiscalYear") or (
                f"FY{period.year}" if period is not None else None
            ),
            revenue_estimate=(
                row.get("revenue_estimate_average")
                if "revenue_estimate_average" in row
                else row.get("estimatedRevenueAvg")
            ),
            estimate_statistic="mean",
            analyst_count=_analyst_count(
                row.get("revenue_estimate_analyst_count")
                if "revenue_estimate_analyst_count" in row
                else row.get("estimatedRevenueAnalystCount")
            ),
            source="alpha_vantage_earnings_estimates",
            source_as_of=source_as_of,
            retrieved_at=_utc_timestamp(retrieved_at),
            available=True,
            reason=None,
            fiscal_period_explicit=period is not None,
            period_frequency="annual",
            revenue_estimate_median=_finite_optional(
                row.get("revenue_estimate_median")
            ),
            revenue_estimate_high=_finite_optional(
                row.get("revenue_estimate_high")
                if "revenue_estimate_high" in row
                else row.get("estimatedRevenueHigh")
            ),
            revenue_estimate_low=_finite_optional(
                row.get("revenue_estimate_low")
                if "revenue_estimate_low" in row
                else row.get("estimatedRevenueLow")
            ),
        ))
    set_as_of_values = [item.source_as_of for item in estimates if item.source_as_of]
    set_as_of = max(set_as_of_values) if set_as_of_values else None
    return build_forward_revenue_estimate_set(
        ticker=issuer_anchor_ticker(ticker),
        latest_actual_fiscal_period=latest_actual_fiscal_period,
        latest_actual_revenue=latest_actual_revenue,
        estimates=tuple(estimates),
        source="alpha_vantage_earnings_estimates",
        source_as_of=set_as_of,
        retrieved_at=retrieved_at,
    )


def _alpha_vantage_rows(payload: dict, frequency: str) -> list[dict]:
    """Support the documented estimates+horizon shape and legacy fixtures."""
    if not isinstance(payload, dict):
        return []
    legacy_key = "annualEstimates" if frequency == "annual" else "quarterlyEstimates"
    if isinstance(payload.get(legacy_key), list):
        return [row for row in payload[legacy_key] if isinstance(row, dict)]
    rows = payload.get("estimates")
    if not isinstance(rows, list):
        return []
    wanted = "fiscal year" if frequency == "annual" else "fiscal quarter"
    return [
        row for row in rows
        if isinstance(row, dict)
        and wanted in str(row.get("horizon", "")).lower()
        and "historical" not in str(row.get("horizon", "")).lower()
    ]


def alpha_vantage_response_to_provider_result(
    *,
    ticker: str,
    payload,
    latest_actual_fiscal_period: pd.Timestamp,
    latest_actual_revenue: float,
    retrieved_at: pd.Timestamp,
    latest_actual_quarterly_period: pd.Timestamp | None = None,
) -> ForwardEstimateProviderResult:
    """Normalize one API response and preserve provider failure semantics."""
    normalized_ticker = issuer_anchor_ticker(ticker)
    retrieved = _utc_timestamp(retrieved_at)
    if not isinstance(payload, dict):
        return ForwardEstimateProviderResult(
            normalized_ticker, normalized_ticker,
            "alpha_vantage_earnings_estimates", None, (), None, retrieved,
            False, "malformed_json", ("provider_response_unusable",),
        )
    error_keys = {
        "Note": "rate_limit_reached",
        "Error Message": "ticker_unavailable",
    }
    for key, reason in error_keys.items():
        if key in payload:
            return ForwardEstimateProviderResult(
                normalized_ticker, normalized_ticker,
                "alpha_vantage_earnings_estimates", None, (), None,
                retrieved, False, reason, (reason,),
            )
    ...
    annual = alpha_vantage_revenue_estimates_to_set(
        ticker=normalized_ticker, payload=payload,
        latest_actual_fiscal_period=latest_actual_fiscal_period,
        latest_actual_revenue=latest_actual_revenue,
        retrieved_at=retrieved,
    )
    quarterly = []
    for row in _alpha_vantage_rows(payload, "quarterly"):
        raw_period = (
            row.get("date") or row.get("fiscalDateEnding")
            or row.get("fiscal_date_ending")
        )
        ...
        is_forward = (
            period is not None
            and (
                latest_actual_quarterly_period is None
                or period > pd.Timestamp(latest_actual_quarterly_period)
            )
        )
        if not is_forward:
            continue
        quarterly.append(ForwardRevenueEstimate(
            normalized_ticker, normalized_ticker, period, None, revenue,
            "mean", analyst_count, "alpha_vantage_earnings_estimates",
            None, retrieved, period is not None and revenue is not None,
            None if period is not None and revenue is not None else
            ("fiscal_period_unavailable" if period is None else
             "invalid_revenue_estimate"),
            (), period is not None, None, "quarterly",
            _finite_optional(row.get("revenue_estimate_median")),
            _finite_optional(row.get("revenue_estimate_high")),
            _finite_optional(row.get("revenue_estimate_low")),
        ))
    duplicate_quarters = {
        item.fiscal_period_end for item in quarterly
        if item.fiscal_period_end is not None
        and sum(other.fiscal_period_end == item.fiscal_period_end
                for other in quarterly) > 1
    }
    if duplicate_quarters:
        quarterly = [
            ...
            ) if item.fiscal_period_end in duplicate_quarters else item
            for item in quarterly
        ]
    quarterly = tuple(sorted(
        quarterly,
        key=lambda item: pd.Timestamp.max if item.fiscal_period_end is None
        else item.fiscal_period_end,
    ))
    warnings = list(annual.warnings)
    if annual.source_as_of is None:
        warnings.append("provider_as_of_unavailable")
    if len([item for item in annual.estimates if item.available]) < 3:
        warnings.append("fewer_than_three_forward_fiscal_years")
    available = bool(
        any(item.available for item in annual.estimates)
        or any(item.available for item in quarterly)
    )
    return ForwardEstimateProviderResult(
        normalized_ticker, normalized_ticker,
        "alpha_vantage_earnings_estimates", annual, quarterly,
        annual.source_as_of, retrieved, available,
        None if available else "forward_estimates_unavailable",
        tuple(dict.fromkeys(warnings)),
    )
